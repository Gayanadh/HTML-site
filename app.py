...
{{"heading": "<chosen heading from the list>", "polished": "<rewritten sentence>"}}"""
...
def find_insert_position_pdf(items, target_heading: str):
    ...
    """
    heading_idx = ATS_HEADING_ORDER.index(target_heading) if target_heading in ATS_HEADING_ORDER else 99
...
def inject_into_pdf(file_bytes: bytes, updates: str, heading: str) -> io.BytesIO:
    ...
    HEADER_H = 16
    CONTENT_H = 30
    BLOCK_H = (CONTENT_H if heading_exists else HEADER_H + CONTENT_H)
...
def _make_xml_para(fill_color, text, bold=False, text_color="1A3A6B", font_size="20", indent=False):
...
def inject_into_docx(file_bytes: bytes, updates: str, heading: str) -> io.BytesIO:
    ...
    if heading_exists:
        ...
        # Insert before the anchor: heading + bullet (insert bullet first, then heading before it)
        bullet = _make_xml_para(
            fill_color="EEF4FF",
            text=f"\u2022  {updates}",
            bold=False,
...
def polish():
    ...
    try:
        ...
        return jsonify({"polished_text": f"AI Error: {str(e)}", "heading": "Achievements"}), 200
...
def upgrade():
    try:
        ...
        if updates and not heading and groq_client:
            result = classify_and_polish(updates, request.form.get("template", "google"))
